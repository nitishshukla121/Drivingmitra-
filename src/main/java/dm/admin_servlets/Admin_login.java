	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String pass=request.getParameter("password");
			...
			if (status==true) 
			{
				...
				HttpSession rs = request.getSession();//creating session for the conection of the web client and web browser
				/*
				 * String sessionId= rs.getId(); System.out.println(sessionId);
				 */
				rs.setAttribute("sessionKey", id);//binding admin id in session
				rs.setAttribute("role", "admin");
				
				response.sendRedirect("/Drivingmitra/admin/admin_home.jsp");
