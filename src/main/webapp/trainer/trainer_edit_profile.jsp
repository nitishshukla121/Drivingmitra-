...
<title>Add Tips for Vehicle - Driving School Arena</title>
...
<%
	...
	if (trainer_id == null || session.isNew()) {
		...
		RequestDispatcher rd = request.getRequestDispatcher("/trainer_login.jsp");
		rd.forward(request, response);
	} else {
		if (role != null && role.equals("trainer")) {
%>

<div class="trainer-hero-container">
    <div class="trainer-hero-content">
        <h1 class="trainer-hero-title animate__animated animate__fadeInDown">Add Vehicle Tips</h1>
        <p class="trainer-hero-subtitle animate__animated animate__fadeInUp animate__delay-0.5s">Share your expertise with trainees.</p>
    </div>
</div>
        ...
        <form action="/Drivingmitra/Add_Tips" method="post">
            <div class="form-group-trainer">
                ...
                <select id="vtype" name="vtype" required>
                    ...
                    <option value="Farm Vehicle">Farm Vehicle</option>
                </select>
            </div>

            <div class="form-group-trainer">
                <label for="tips">Tips:</label>
                <textarea id="tips" name="tips" placeholder="Enter tips here" required></textarea>
            </div>

            <button type="submit" class="btn-submit-trainer">Submit Tips</button>
        </form>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Set the background image on the body for a consistent look
